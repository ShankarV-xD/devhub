...
export default function ToolButton({
  ...
  const tooltipText = lastUsedTimestamp
    ? `Last used ${formatTimeAgo(lastUsedTimestamp)}`
    : undefined;

  const finalTooltipText = title || tooltipText || label;
  ...
  const button = (
    <button
      ...
      className={clsx(
        "flex items-center gap-3 rounded-lg font-medium transition-all duration-200 active:scale-95 w-full cursor-pointer group relative",
        sizeClasses[size],
        variant === "danger"
    ...
    >
      ...
      {isLoading ? (
        ...
        React.cloneElement(icon as React.ReactElement<any>, {
          ...
          strokeWidth: (icon.props as any).strokeWidth || 2,
        })
      ) : (
        icon
      )}
